    ...
    <PatientPage title="Find your care team" eyebrow="My doctors">
        ...
        <div className="flex flex-col gap-3 md:flex-row">
          <label className="relative block flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-[#e9e2dc] bg-[#fffdfb] py-3 pl-11 pr-4 text-sm outline-none transition focus:border-[#d98268]"
            />
          </label>
          <select
            value={specialty}
            onChange={(event) => setSpecialty(event.target.value)}
            className="rounded-xl border border-[#e9e2dc] bg-[#fffdfb] px-4 py-3 text-sm text-[#3b4944] outline-none focus:border-[#d98268]"
          >
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-2">
        {filteredDoctors.map((doctor) => (
          <article
            key={doctor.id}
            className="group flex flex-col overflow-hidden rounded-4xl border border-[#eadfd9] bg-white shadow-[0_10px_30px_rgba(125,79,62,.05)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(125,79,62,.12)] sm:flex-row"
          >
            <div className="relative h-56 shrink-0 overflow-hidden sm:h-auto sm:w-40">
              <img
                src={doctor.image}
                alt={doctor.name}
                className="h-full min-h-56 w-full object-cover object-top transition duration-500 group-hover:scale-105"
              />
              <div className="absolute right-3 top-3 flex items-center gap-1 rounded-full bg-white px-2.5 py-1 text-xs font-bold text-[#3b4944] shadow">
                <Star
                  size={13}
                  fill="currentColor"
                  className="text-[#b38336]"
                />
                {doctor.rating}
              </div>
            </div>
            <div className="flex min-w-0 flex-1 flex-col p-5 sm:p-6">
              <div>
                <h2 className="font-serif text-2xl">{doctor.name}</h2>
                <p className="mt-1 font-semibold text-[#c87861]">
                  {doctor.specialty}
                </p>
              ...
              <div className="mt-5 flex flex-wrap items-end justify-between gap-4 border-t border-[#f0e8e3] pt-4">
